...
Since hook length (for a given $n$) depends only on the value $i+j$, which , we can simplify a bit. There is always just one block where that value is $2$ (the first block), two where it is $3$, and so on up to $n$ blocks where it is $n+1$. And where $k$ is  
